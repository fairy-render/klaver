...
use super::{
    ...
    primordials::BasePrimordials,
    tuples::Pair,
};

pub type TypedListEntries<'js, T> = FromJsIter<JsIterator<'js>, Pair<usize, T>>;

pub type TypedListValues<'js, T> = FromJsIter<JsIterator<'js>, T>;
    ...
    T: FromJs<'js> + IntoJs<'js>,
{
    ...
    pub fn entries(&self) -> rquickjs::Result<TypedListEntries<'js, T>> {
        let iter: JsIterator<'js> = self.i.call_property(
            BasePrimordials::from_ctx(self.i.ctx())?
                .atom_entries
                .clone(),
            (),
        )?;
    ...
    pub fn values(&self) -> rquickjs::Result<TypedListValues<'js, T>> {
        let iter: JsIterator<'js> = self.i.call_property(PredefinedAtom::Values, ())?;
        Ok(iter.from_javascript())
    }
...
#[cfg(test)]
mod tests {
    use super::super::iterable::NativeIteratorInterface;
