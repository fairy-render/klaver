...
impl<'js> Set<'js> {
    pub fn new(ctx: &Ctx<'js>) -> rquickjs::Result<Set<'js>> {
        let obj = BasePrimordials::from_ctx(ctx)?.construct_set(())?;
        Ok(Self { object: obj })
    }
    ...
    pub fn is(ctx: &Ctx<'js>, value: &rquickjs::Value<'js>) -> rquickjs::Result<bool> {
        ...
        Ok(obj.is_instance_of(&BasePrimordials::from_ctx(ctx)?.constructor_set))
    }
    ...
    pub fn entries(&self) -> rquickjs::Result<JsIterator<'js>> {
        let iter = self
            .object
            .call_property::<_, _, JsIterator<'js>>("entries", ())?;
