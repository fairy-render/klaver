...
use super::{
    Map,
    iterable::{FromJsIter, JsIterator, NativeIteratorExt},
    tuples::Pair,
};

pub type TypedMapEntries<'js, K, V> = FromJsIter<JsIterator<'js>, Pair<K, V>>;

pub type TypedMapValues<'js, V> = FromJsIter<JsIterator<'js>, V>;

pub type TypedMapKeys<'js, K> = FromJsIter<JsIterator<'js>, K>;
    ...
    K: IntoJs<'js> + FromJs<'js>,
{
    ...
    pub fn entries(&self) -> rquickjs::Result<FromJsIter<JsIterator<'js>, Pair<K, T>>> {
        Ok(self.i.entries()?.from_javascript())
    }

    pub fn values(&self) -> rquickjs::Result<FromJsIter<JsIterator<'js>, T>> {
        Ok(self.i.values()?.from_javascript())
    }

    pub fn keys(&self) -> rquickjs::Result<FromJsIter<JsIterator<'js>, K>> {
