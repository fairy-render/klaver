...
use super::{ObjectId, TransferData, context::SerializationContext};
...
impl Registry {
    pub fn new(ctx: &Ctx<'_>) -> rquickjs::Result<Registry> {
        let registry = Registry {
            types: Default::default(),
        };

        registry.register::<i64>(ctx)?;
        registry.register::<f64>(ctx)?;
        registry.register::<bool>(ctx)?;
        registry.register::<Date>(ctx)?;
        registry.register::<rquickjs::String>(ctx)?;
        registry.register::<rquickjs::Object>(ctx)?;
        registry.register::<rquickjs::Array>(ctx)?;
    ...
    pub fn get_by_tag(&self, ctx: &Ctx<'_>, tag: &Tag) -> rquickjs::Result<Cloner> {
        let lock = throw_if!(ctx, self.types.read()); // self.types.read().expect("Lock");
    ...
    pub fn register<T>(&self, ctx: &Ctx<'_>) -> rquickjs::Result<()>
    where
        T: Clonable,
        T::Cloner: Send + Sync,
    {
        let mut lock = throw_if!(ctx, self.types.write());

        if lock.contains_key(T::Cloner::tag()) {
            throw!(@type ctx, format!("Tag '{}' already defined in registry", T::Cloner::tag()));
        }
