use klaver_shared::{
    buffer::Buffer,
    iter::DynamicStream,
    util::{is_async_iterator, is_iterator},
};
use rquickjs::{
    atom::PredefinedAtom, class::Trace, Class, Ctx, FromJs, Function, Object, Promise, Symbol,
    Value,
};
...
impl<'js> Iter<'js> {
    pub fn pull(&self, ctx: &Ctx<'js>) -> rquickjs::Result<Chunk<'js>> {
        let ret = self
            .i
            .get::<_, Function>(PredefinedAtom::Next)?
            .call::<_, Value>(())?;
...
impl<'js> DynamicStream<'js> for Iter<'js> {
    ...
    where
        'js: 'a,
    {
        Box::pin(async move { Ok(self.pull(&ctx)?.value) })
    }
}
