...
impl Inner {
    fn notify_shutdown(&self, parent: AsyncId) -> rquickjs::Result<()> {
        ...
        task.shutdown.notify(usize::MAX);
...
impl ExecState {
    ...
    {
        {
            ...
            if let Some(current) = inner.current_id.last().copied() {
                ...
                inner.execution_id.push(id);
            }
            inner.current_id.push(id);
        }
        self.0.borrow_mut().current_id.push(id);
        let ret = func();
    ...
    pub async fn kill(&self, id: AsyncId) -> rquickjs::Result<()> {
        if self.child_count(id) == 0 {
            return Ok(());
        }

        self.0.borrow().kill.notify(usize::MAX.tag(id));
