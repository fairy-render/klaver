...
use super::utils::export_default;
...
pub fn free_timers(rt: *mut qjs::JSRuntime) {
    let mut rt_timers = RUNTIME_TIMERS.lock().unwrap();

    let idx = rt_timers
        .iter()
        .enumerate()
        .find_map(|(idx, m)| if m.rt == rt { Some(idx) } else { None });

    let Some(idx) = idx else { return };

    let state = rt_timers.remove(idx);

    for time_ref in state.timers {
        if let Some(cb) = time_ref.callback {
            let ctx2 = unsafe { Ctx::from_raw(time_ref.ctx) };
            cb.restore(&ctx2).ok();
        }
    }
}
