...
`console`, `fetch`/`Request`/`Response`/`Headers`/`URL`/`URLSearchParams`/`FormData`, timers, Web
Crypto, `Intl`, streams (`ReadableStream`/`WritableStream`/`TransformStream`), `EventTarget`/`Event`,
`MessageChannel`, `Worker`, a sandboxed filesystem, `Blob`/`File`, text encoding, and more — plus
