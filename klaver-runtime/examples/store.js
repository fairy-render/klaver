...
const snapshot = storage.run(new Map(), () => {
  ...
  setTimeout(() => {
    print(
      "Inside first timeout executionAsyncId:",
      executionAsyncId(),
      "triggerAsyncId:",
      triggerAsyncId(),
    );
    ...
    setTimeout(() => {
      print(
        "Inside second timeout executionAsyncId:",
        executionAsyncId(),
        "triggerAsyncId:",
        triggerAsyncId(),
      );
...
const future = snapshot(() => {
  ...
  setTimeout(() => {
    print(
      "Snapshot timeout",
      executionAsyncId(),
      storage.getStore().get("key1"),
    );
  }, 100);

  return delay(500).then(() => {
    print(
      "Snap After delay executionAsyncId:",
      executionAsyncId(),
      "triggerAsyncId:",
      triggerAsyncId(),
    );
