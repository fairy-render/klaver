...
use crate::{ObjectExt, iterable::JsIterator, primordials::BasePrimordials};
...
impl<'js> Map<'js> {
    pub fn new(ctx: &Ctx<'js>) -> rquickjs::Result<Map<'js>> {
        let obj = BasePrimordials::from_ctx(ctx)?.construct_map(())?;
        Ok(Self { object: obj })
    }
    ...
    pub fn is(ctx: &Ctx<'js>, value: &rquickjs::Value<'js>) -> rquickjs::Result<bool> {
        let map_ctor = BasePrimordials::from_ctx(ctx)?.constructor_map.clone();
    ...
    pub fn entries(&self) -> rquickjs::Result<JsIterator<'js>> {
        let iter = self.object.call_property::<_, _, JsIterator<'js>>(
            BasePrimordials::from_ctx(self.object.ctx())?
                .atom_entries
                .clone(),
            (),
        )?;
    ...
    pub fn values(&self) -> rquickjs::Result<JsIterator<'js>> {
        let iter = self
            .object
            .call_property::<_, _, JsIterator<'js>>(PredefinedAtom::Values, ())?;

        Ok(iter)
    }

    pub fn keys(&self) -> rquickjs::Result<JsIterator<'js>> {
        let iter = self.object.call_property::<_, _, JsIterator<'js>>(
            BasePrimordials::from_ctx(self.object.ctx())?
                .atom_keys
                .clone(),
